...
from
    select 
        ...
        date, 
        store_address 
    from login_attendances as time_in_table 
    GROUP BY 
        time_in_table.employee_name, time_in_table.date, time_in_table.store_address 
join (
    select max(time) as time_out, employee_name, date, store_address from login_attendances as time_out_table GROUP BY employee_name, date, store_address
) on time_in_table.date = time_out_table.date

select max(time) as time_out, employee_name, date, store_address from login_attendances GROUP BY employee_name, date, store_address;
