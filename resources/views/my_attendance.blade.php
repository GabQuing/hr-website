...
<div class="u-box u-box-shadow-medium">
    ...
    <div style="display: flex; flex-direction: column; gap: 1rem;">
        ...
        <div class="u-mt-32 u-p-10">
            <div>
                <span class="text-sky-blue u-fw-b u-fs-small">{{ auth()->user()->name; }}</span>
            </div>
            <div class="dashboard_table mh-500 u-flex u-p-10 custom-grid-container u-mt-5">
                ...
                <div class="u-flex u-align-items-center u-flex-direction-column">
                    ...
                    <div class="attendance-graph mh-200" data-user="{{ auth()->user()->id }}"
                        ...
                        <div class="days-grid">
                            ...
                            @for($day = 1; $day <= $daysInMonth; $day++) <div
                                class="day tooltip user-{{ auth()->user()->id }} day-{{ $year }}-{{ str_pad($monthNumber, 2, '0', STR_PAD_LEFT) }}-{{ str_pad($day, 2, '0', STR_PAD_LEFT) }}"
                                data-user="{{ auth()->user()->id }}" data-date="{{ $month }}-{{ $day }}-{{ $year }}">
                        </div>
                        @endfor
                    </div>
                </div>
                <div style="margin-top: 2rem"
                    class="calendar-loader loader-{{ $month }} user-{{ auth()->user()->id }} year-{{ $year }} month-{{ $month }}">
...
<script>
    ...
    @if ($summary_data ?? false)
    // attendance graph
    const WORK_SCHEDULES = {!! json_encode(auth()->user()->workSchedule) !!};
    ...
    function populateCalendar(month, data, formattedDate, attendanceGraph) {
        ...
        for (let i=1; i<=31; i++) {
            ...
            const workSchedule = daylog?.work_schedule.work_from ?? WORK_SCHEDULES.find(e => e.work_day === dayName);
